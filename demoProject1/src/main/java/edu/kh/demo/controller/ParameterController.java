...
public class ParameterController {
	...
	public String paramTest2(@RequestParam("title") String title,
							 ...
							 @RequestParam(value="publisher", required = false, defaultValue = "kh 출판사") String publisher) {
	...
	public String paramTest3(@RequestParam("color") String[] colorArr,
							 @RequestParam("fruit") List<String> friuitList,
							 @RequestParam Map<String, Object> paramMap) {
		
		log.debug("colorArr : " + Arrays.toString(colorArr));
		
		log.debug("friuitList : " + friuitList);
		...
		// paramMap : {color=Red, fruit=Apple, productName=집, expirationDate=2025-04-03}
		
		return "redirect:/param/main";
	}
	
	// 4. @ModerlAttribute 를 이용한 파라미터 얻어오기
	// @ModerlAttribute
	...
	public String paramTest4(/*@ModelAttribute*/ MemberDTO inputMember) {
		...
		// 상대경로 작성법 (현재위치가 중요! -> 현재 경로의 가장 마지막 부분을 redirect:이주소 로 변경하기 때문에)
	}
