...
VALUES(1, 1998); -- 1번 회원이 1994번 게시글에 좋아요를 누름
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람 오픈멤버일때는 참가자 타겟번호)
		R.OPEN_MEMBER -- NULL 인 경우 (내가 오픈멤버가 아닐때는 OPEN_MEMBER가 타겟번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
SELECT MEMBER_NO, MEMBER_EMAIL, MEMBER_NICKNAME, ENROLL_DATE, COUNT(*) AS SIGN_COUNT
FROM "MEMBER"
WHERE (SYSDATE - ENROLL_DATE) <= 7
GROUP BY MEMBER_NO, MEMBER_EMAIL, MEMBER_NICKNAME, ENROLL_DATE
