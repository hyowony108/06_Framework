...
// 타이머 역할을 할 setInterval 함수를 저장할 변수
let authTimer; 
...
// 2) 이메일 입력(input)될 때 마다 유효성 검사 수행
...
memberPw.addEventListener("input" , e => {
  ...
  if(inputPw.trim().length === 0) {
    ...
    pwMessage.classList.remove("comfirm", "error");
  ...
  // 비밀번호 임력 시 비밀번호 확인락의 값과 비교하는 코드 추가
...
signUpForm.addEventListener("submit", e => {
  ...
  for(let key in checkobj) { // checkObj 요소의 key값을 순서대로 꺼내옴
    ...
    if( !checkobj[key] ){ // 현재 접근중인 checkObj[key]의 value 값이 false인 경우
      ...
      document.getElementById(key).focus(); // 해당 input에 초점 이동
