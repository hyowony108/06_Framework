...
// 기존 닉네임과 새로 입력된 닉네임이 다르면 중복검사 시도학
...
/*
submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수 실행중 일 때
...
if(profileForm != null) {
	...
	// http://locslhost/image/user.png

	let statusCheck = -1; // -1 : 초기상태, 0 : 이미지 삭제, 1 : 새 이미지 선택

	// img 태그에 작성하는 값 src = 미리보기 이미지를 띄울 URL 주소
	let previousImage = profileImg.scr; // 이전 이미지 URL 기록 (초기 상태의 이미지 URL 저장)
	...
	imageInput.addEventListener("change", () => {
		...
		if(file) { // 파일이 선택된 경우
			...
			if(file.size <= MAX_SIZE){ // 현재 선택한 파일의 크기가 허용범위 이내인 경우(정상인 경우)
				...
				previousImage = newImageUrl; //  현재 선택된 이미지를 이전 이미지로 저장(다임에 바뀔일에 대비)
				previousFile = file; //  현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뀔일에 대비)
				statusCheck = 1; // 새 이미지 선택 상태 기돌
		...
		profileImg.src = previousImage;
			...
			if(previousFile){ // 이전 파일이 존재한다면
				...
				// --> input 태그의 files 속성은 FileList 만 저장 가능한 형태이기 때문에
