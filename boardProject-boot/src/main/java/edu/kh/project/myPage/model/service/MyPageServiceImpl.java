...
public class MyPageServiceImpl implements MyPageService{
	...
	public int changePw(Map<String, String> paramMap, int memberNo) {
		...
		// 같을 경우
		// 새 비밀번호를 안호화 (bcrypt.encode(평문))
		String encPw = bcrypt.encode(paramMap.get("newPw"));
		
		// DB에 업데이트
		// SQL 전달해야하는 데이터 2개 (암호화한 새 비번 encPw, 회원번호)
	...
	public int fileUpload2(MultipartFile uploadFile, int memberNo) throws Exception {
		...
		// 파일이 저장될 서버 폴더 경로
		String floderPath = "C:/uploadFiles/test/";
		...
		int result = mapper.insertUplodeFile(uf);
		...
		// 파일을 서버 컴퓨터에 저장!
		uploadFile.transferTo(new File(floderPath + fileRename));
			// C:/uploadFiles/test/20240424150830_0001.jpg
	...
	public int fileUpload3(List<MultipartFile> aaaList, List<MultipartFile> bbbList, int memberNo) throws Exception {
		...
		// 1. aaaList 처리
		int result1 = 0; // 결과(INSERT된 행의 갯수) 저장용 변수
		...
		// 2. bbbList 처리
		int result2 = 0; // 결과(INSERT된 행의 갯수) 저장용 변수
