...
public class FileConfig implements WebMvcConfigurer{
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		// 서버폴더로 경로 중 C:/uploadFiles/test/ 로 연결하겠다
	...
	public MultipartConfigElement configElement() {
		...
		// 업로드 파일의 최대 크기, 메모리에서 임시 저장경로 등 설정 가능
	...
	public MultipartResolver multipartResolver() {
		// MultipartResolver : MultipartFile 을 처리해주는 해결자
		// MultipartResolver는 클라이언트로부터 받은 Multipart 요청을 처리하고,
		// 이 중에서 업로드된 파일을 추출하여 MultipartFile 객체로 제공하는 역할
		StandardServletMultipartResolver mulitipartResolver 
		= new StandardServletMultipartResolver();
				
		return mulitipartResolver;
